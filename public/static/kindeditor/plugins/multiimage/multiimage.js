	...
	init : function(options) {
		...
		var settings = {
			...
			upload_success_handler : function(file, serverData) {
				...
				file.url = data.obj.url;
				K('.ke-img', itemDiv).attr('src', imageServerAddr + file.url).attr('data-status', file.filestatus).data('data', data);
				K('.ke-status > div', itemDiv).hide();
			}
		};
		...
		self.plugin.multiImageDialog({
			clickFn : function (urlList) {
				...
				K.each(urlList, function(i, data) {
					if (self.afterUpload) {
						self.afterUpload.call(self, data.obj.url, data, 'multiimage');
					}
					self.exec('insertimage', data.obj.url, data.title, data.width, data.height, data.border, data.align);
				});
